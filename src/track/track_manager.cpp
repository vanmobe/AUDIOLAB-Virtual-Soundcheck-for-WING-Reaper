...
bool TrackManager::SetTrackHardwareOutput(MediaTrack* track, int output_index, int num_channels) {
    ...
    // Set track channel count BEFORE creating the send — REAPER uses this at
    // CreateTrackSend time to determine whether the send is mono or stereo.
    SetMediaTrackInfo_Value(track, "I_NCHAN", num_channels >= 2 ? 2 : 1);
    ...
    if (num_channels >= 2) {
        ...
        // Mono: bit 10 on both SRCCHAN and DSTCHAN marks single-channel routing.
        // This mirrors I_RECINPUT encoding but inverted: for sends, bit 10 set = mono.
        SetTrackSendInfo_Value(track, 1, send_idx, "I_SRCCHAN", 0 | (1 << 10));
        SetTrackSendInfo_Value(track, 1, send_idx, "I_DSTCHAN", output_index | (1 << 10));
    }
