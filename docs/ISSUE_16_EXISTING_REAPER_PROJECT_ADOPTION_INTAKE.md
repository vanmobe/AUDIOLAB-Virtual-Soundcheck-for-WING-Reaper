...
- UI/UX: macOS can support a dedicated adoption wizard. Windows fallback should stay simpler at first, likely summary-and-confirm rather than a dense inline editor.
...
- How should the fallback UI work on Windows if the mapping review is too complex for a single REAPER input dialog?
